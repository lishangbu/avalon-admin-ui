...
import { SkillChargeSkipWeathersPage } from './SkillChargeSkipWeathersPage';

beforeEach(() => {
  mockBattleRulePageRequests();
});

it('renders skill charge skip weather rules with readable references and opens the editor', async () => {
  const user = userEvent.setup();
  renderWithQuery(<SkillChargeSkipWeathersPage />);

  expect(screen.getByRole('heading', { name: '技能蓄力天气' })).toBeInTheDocument();
  expect(await screen.findByText('十万伏特 / 标准伤害')).toBeInTheDocument();
  expect(screen.getByText('下雨')).toBeInTheDocument();

  await user.click(screen.getByRole('button', { name: /新建规则/ }));

  await waitFor(() => expect(screen.getByText('新建技能蓄力天气规则')).toBeInTheDocument());
