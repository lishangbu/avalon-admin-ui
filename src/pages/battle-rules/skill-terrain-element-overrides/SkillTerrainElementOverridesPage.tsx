...
import {
  battleRulesServices,
  type BattleSkillTerrainElementOverrideRequest,
  type BattleSkillTerrainElementOverrideResponse,
...
export function SkillTerrainElementOverridesPage() {
  const [filters, setFilters] = useState<SkillTerrainElementOverrideFilters>({});
  ...
  const [editingRecord, setEditingRecord] =
    useState<BattleSkillTerrainElementOverrideResponse | null>(null);
  const [form] = Form.useForm<BattleSkillTerrainElementOverrideRequest>();
  const queryClient = useQueryClient();
  const options = useBattleRuleOptions(['skillRules', 'terrainRules', 'elements']);
  ...
  const overridesQuery = useQuery({
    queryKey: ['battle-rules', 'skill-terrain-element-overrides', query],
    queryFn: () => battleRulesServices.skillTerrainElementOverrides.list(query),
  });

  const saveMutation = useMutation({
    mutationFn: (values: BattleSkillTerrainElementOverrideRequest) => {
      if (modalMode === 'create') {
        return battleRulesServices.skillTerrainElementOverrides.create(values);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的技能场地属性覆盖');
      }
      return battleRulesServices.skillTerrainElementOverrides.update(editingRecord.id, values);
    },
    onSuccess: async () => {
      message.success('技能场地属性覆盖已保存');
      closeModal();
      await queryClient.invalidateQueries({
        queryKey: ['battle-rules', 'skill-terrain-element-overrides'],
      });
    },
    onError: (error) => message.error(apiErrorMessage(error, '保存技能场地属性覆盖失败')),
  });

  const removeMutation = useMutation({
    mutationFn: (record: BattleSkillTerrainElementOverrideResponse) =>
      battleRulesServices.skillTerrainElementOverrides.remove(record.id),
    onSuccess: async () => {
      message.success('技能场地属性覆盖已删除');
      await queryClient.invalidateQueries({
        queryKey: ['battle-rules', 'skill-terrain-element-overrides'],
      });
    },
    onError: (error) => message.error(apiErrorMessage(error, '删除技能场地属性覆盖失败')),
  });

  const columns: ColumnsType<BattleSkillTerrainElementOverrideResponse> = [
    {
      title: '技能规则',
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <Typography.Text type="secondary">维护技能在指定场地下改用的结算属性。</Typography.Text>
        </div>
      ...
      <Card size="small">
        <Table<BattleSkillTerrainElementOverrideResponse>
          rowKey="id"
          columns={columns}
      ...
      <Modal
        open={modalOpen}
        title={modalMode === 'create' ? '新建技能场地属性覆盖' : '编辑技能场地属性覆盖'}
        okText="保存"
        cancelText="取消"
  ...
  function updateFilter(next: Partial<SkillTerrainElementOverrideFilters>) {
  ...
  function openEdit(record: BattleSkillTerrainElementOverrideResponse) {
    setModalMode('edit');
