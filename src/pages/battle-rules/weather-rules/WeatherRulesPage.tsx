...
import {
  ...
  type BattleWeatherRuleRequest,
  type BattleWeatherRuleResponse,
...
interface WeatherRuleFilters {
  q: string;
}

export function WeatherRulesPage() {
  const [filters, setFilters] = useState<WeatherRuleFilters>({ q: '' });
  const [page, setPage] = useState(defaultPageState);
  const [modalOpen, setModalOpen] = useState(false);
  const [modalMode, setModalMode] = useState<BattleRuleModalMode>('create');
  const [editingRecord, setEditingRecord] = useState<BattleWeatherRuleResponse | null>(null);
  const [form] = Form.useForm<BattleWeatherRuleRequest>();
  ...
  const weatherQuery = useQuery({
    queryKey: ['battle-rules', 'weather-rules', query],
    queryFn: () => battleRulesServices.weatherRules.list(query),
  });

  const saveMutation = useMutation({
    mutationFn: (values: BattleWeatherRuleRequest) => {
      const payload = compactRulePayload(values);
      if (modalMode === 'create') {
        return battleRulesServices.weatherRules.create(payload);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的天气规则');
      }
      return battleRulesServices.weatherRules.update(editingRecord.id, payload);
    },
    onSuccess: async () => {
      message.success('天气规则已保存');
      closeModal();
      await queryClient.invalidateQueries({ queryKey: ['battle-rules', 'weather-rules'] });
    },
    onError: (error) => message.error(apiErrorMessage(error, '保存天气规则失败')),
  });

  const removeMutation = useMutation({
    mutationFn: (record: BattleWeatherRuleResponse) =>
      battleRulesServices.weatherRules.remove(record.id),
    onSuccess: async () => {
      message.success('天气规则已删除');
      await queryClient.invalidateQueries({ queryKey: ['battle-rules', 'weather-rules'] });
    },
    onError: (error) => message.error(apiErrorMessage(error, '删除天气规则失败')),
  });

  const columns: ColumnsType<BattleWeatherRuleResponse> = [
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <Typography.Text type="secondary">维护战斗天气及其默认持续回合。</Typography.Text>
        </div>
      ...
      <Card size="small">
        <Table<BattleWeatherRuleResponse>
          rowKey="id"
          columns={columns}
          dataSource={toPageRows(weatherQuery.data)}
          loading={weatherQuery.isLoading || weatherQuery.isFetching}
          scroll={{ x: 1180 }}
          pagination={{
            current: page.current,
            pageSize: page.pageSize,
            total: toPageTotal(weatherQuery.data),
      ...
      <Modal
        open={modalOpen}
        title={modalMode === 'create' ? '新建天气规则' : '编辑天气规则'}
        okText="保存"
        cancelText="取消"
  ...
  function updateFilter(next: Partial<WeatherRuleFilters>) {
  ...
  function openEdit(record: BattleWeatherRuleResponse) {
    setModalMode('edit');
