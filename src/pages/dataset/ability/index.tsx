...
import type { AbilityRecord, AbilityQuery, AbilityUpsertInput } from './service'
...
const pageTitle = '特性管理'
const pageSubtitle = '对接后端特性接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): AbilityQuery {
  ...
  if (values.introduction.trim()) {
    query.introduction = values.introduction.trim()
  }

  return query
}

function toFormValues(record?: AbilityRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): AbilityUpsertInput {
  const payload: AbilityUpsertInput = {}
  ...
  payload.introduction = values.introduction.trim()

  return payload
}

export default function DatasetAbilityPage() {
  ...
  const [editingRow, setEditingRow] = useState<AbilityRecord | null>(null)
  const [query, setQuery] = useState<AbilityQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'ability', 'list', query],
  ...
  async function loadData(nextQuery: AbilityQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'ability', 'list', nextQuery],
  ...
  function openEdit(record: AbilityRecord) {
  ...
  async function handleDelete(record: AbilityRecord) {
      ...
      <Card style={{ marginBottom: 16 }}>
        <Form form={searchForm} layout="inline" onFinish={handleSearchSubmit}>
          ...
          <Form.Item name="introduction" label="介绍">
            <Input allowClear placeholder="请输入介绍" />
          </Form.Item>
          <Form.Item>
            <Space>
