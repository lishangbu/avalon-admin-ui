...
import type { GenderRecord, GenderQuery, GenderUpsertInput } from './service'
...
const pageTitle = '性别管理'
const pageSubtitle = '对接后端性别接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): GenderQuery {
  const query: GenderQuery = {}
...
function toFormValues(record?: GenderRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): GenderUpsertInput {
  const payload: GenderUpsertInput = {}
...
export default function DatasetGenderPage() {
  ...
  const [editingRow, setEditingRow] = useState<GenderRecord | null>(null)
  const [query, setQuery] = useState<GenderQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'gender', 'list', query],
  ...
  async function loadData(nextQuery: GenderQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'gender', 'list', nextQuery],
  ...
  function openEdit(record: GenderRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('性别已更新')
      } else {
        await createRow(payload)
        message.success('性别已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  async function handleDelete(record: GenderRecord) {
  ...
  const columns: ColumnsType<GenderRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: GenderRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<GenderRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'gender-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
