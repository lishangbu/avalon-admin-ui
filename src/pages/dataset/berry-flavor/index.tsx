...
function toSearchQuery(values: SearchValues): BerryFlavorQuery {
  const query: BerryFlavorQuery = {}
...
function toPayload(values: FormValues): BerryFlavorUpsertInput {
  const payload: BerryFlavorUpsertInput = {}
...
export default function DatasetBerryFlavorPage() {
  ...
  const [editingRow, setEditingRow] = useState<BerryFlavorRecord | null>(null)
  const [query, setQuery] = useState<BerryFlavorQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'berry-flavor', 'list', query],
  ...
  async function loadData(nextQuery: BerryFlavorQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'berry-flavor', 'list', nextQuery],
  ...
  function openEdit(record: BerryFlavorRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('树果风味已更新')
      } else {
        await createRow(payload)
        message.success('树果风味已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  const columns: ColumnsType<BerryFlavorRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
