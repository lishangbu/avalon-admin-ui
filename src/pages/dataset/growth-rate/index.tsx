...
import type { GrowthRateRecord, GrowthRateQuery, GrowthRateUpsertInput } from './service'
...
const pageTitle = '成长速率管理'
const pageSubtitle = '对接后端成长速率接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): GrowthRateQuery {
  const query: GrowthRateQuery = {}
...
function toFormValues(record?: GrowthRateRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): GrowthRateUpsertInput {
  const payload: GrowthRateUpsertInput = {}
...
export default function DatasetGrowthRatePage() {
  ...
  const [editingRow, setEditingRow] = useState<GrowthRateRecord | null>(null)
  const [query, setQuery] = useState<GrowthRateQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'growth-rate', 'list', query],
  ...
  async function loadData(nextQuery: GrowthRateQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'growth-rate', 'list', nextQuery],
  ...
  function openEdit(record: GrowthRateRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('成长速率已更新')
      } else {
        await createRow(payload)
        message.success('成长速率已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  async function handleDelete(record: GrowthRateRecord) {
  ...
  const columns: ColumnsType<GrowthRateRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: GrowthRateRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<GrowthRateRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'growth-rate-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
      ...
      <Modal
        destroyOnHidden
        title={editingRow ? '编辑成长速率' : '新增成长速率'}
        open={modalOpen}
        width={modalWidth}
