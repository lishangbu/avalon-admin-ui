...
import type { RegionRecord, RegionQuery, RegionUpsertInput } from './service'
...
const pageTitle = '地区管理'
const pageSubtitle = '对接后端地区接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): RegionQuery {
...
function toFormValues(record?: RegionRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): RegionUpsertInput {
  const payload: RegionUpsertInput = {}
...
export default function DatasetRegionPage() {
  ...
  const [editingRow, setEditingRow] = useState<RegionRecord | null>(null)
  const [query, setQuery] = useState<RegionQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'region', 'list', query],
  ...
  async function loadData(nextQuery: RegionQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'region', 'list', nextQuery],
  ...
  function openEdit(record: RegionRecord) {
  ...
  async function handleDelete(record: RegionRecord) {
  ...
  const columns: ColumnsType<RegionRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: RegionRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<RegionRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'region-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
