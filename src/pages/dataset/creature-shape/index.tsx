...
import type { CreatureShapeRecord, CreatureShapeQuery, CreatureShapeUpsertInput } from './service'
...
const pageTitle = '精灵形状管理'
const pageSubtitle = '对接后端精灵形状接口，支持列表查询、新增、编辑和删除。'
  ...
  return (
    ...
    >
      ...
      <Table<CreatureShapeRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'creature-shape-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
