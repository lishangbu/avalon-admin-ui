...
import type { CreatureColorRecord, CreatureColorQuery, CreatureColorUpsertInput } from './service'
...
const pageTitle = '精灵颜色管理'
const pageSubtitle = '对接后端精灵颜色接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): CreatureColorQuery {
  const query: CreatureColorQuery = {}
...
function toFormValues(record?: CreatureColorRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): CreatureColorUpsertInput {
  const payload: CreatureColorUpsertInput = {}
...
export default function DatasetCreatureColorPage() {
  ...
  const [editingRow, setEditingRow] = useState<CreatureColorRecord | null>(null)
  const [query, setQuery] = useState<CreatureColorQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'creature-color', 'list', query],
  ...
  async function loadData(nextQuery: CreatureColorQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'creature-color', 'list', nextQuery],
  ...
  function openEdit(record: CreatureColorRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('精灵颜色已更新')
      } else {
        await createRow(payload)
        message.success('精灵颜色已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  async function handleDelete(record: CreatureColorRecord) {
  ...
  const columns: ColumnsType<CreatureColorRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: CreatureColorRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<CreatureColorRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'creature-color-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
      ...
      <Modal
        destroyOnHidden
        title={editingRow ? '编辑精灵颜色' : '新增精灵颜色'}
        open={modalOpen}
        width={modalWidth}
