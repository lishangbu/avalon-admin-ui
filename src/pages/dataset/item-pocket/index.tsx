...
import type { ItemPocketRecord, ItemPocketQuery, ItemPocketUpsertInput } from './service'
...
const pageTitle = '道具口袋管理'
const pageSubtitle = '对接后端道具口袋接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): ItemPocketQuery {
  const query: ItemPocketQuery = {}
...
function toFormValues(record?: ItemPocketRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): ItemPocketUpsertInput {
  const payload: ItemPocketUpsertInput = {}
...
export default function DatasetItemPocketPage() {
  ...
  const [editingRow, setEditingRow] = useState<ItemPocketRecord | null>(null)
  const [query, setQuery] = useState<ItemPocketQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'item-pocket', 'list', query],
  ...
  async function loadData(nextQuery: ItemPocketQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'item-pocket', 'list', nextQuery],
  ...
  function openEdit(record: ItemPocketRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('道具口袋已更新')
      } else {
        await createRow(payload)
        message.success('道具口袋已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  async function handleDelete(record: ItemPocketRecord) {
  ...
  const columns: ColumnsType<ItemPocketRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: ItemPocketRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<ItemPocketRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'item-pocket-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
      ...
      <Modal
        destroyOnHidden
        title={editingRow ? '编辑道具口袋' : '新增道具口袋'}
        open={modalOpen}
        width={modalWidth}
