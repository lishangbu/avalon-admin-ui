...
import type { ItemAttributeRecord, ItemAttributeQuery, ItemAttributeUpsertInput } from './service'
...
const pageTitle = '道具属性管理'
const pageSubtitle = '对接后端道具属性接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): ItemAttributeQuery {
  const query: ItemAttributeQuery = {}
...
function toFormValues(record?: ItemAttributeRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): ItemAttributeUpsertInput {
  const payload: ItemAttributeUpsertInput = {}
...
export default function DatasetItemAttributePage() {
  ...
  const [editingRow, setEditingRow] = useState<ItemAttributeRecord | null>(null)
  const [query, setQuery] = useState<ItemAttributeQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'item-attribute', 'list', query],
  ...
  async function loadData(nextQuery: ItemAttributeQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'item-attribute', 'list', nextQuery],
  ...
  function openEdit(record: ItemAttributeRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('道具属性已更新')
      } else {
        await createRow(payload)
        message.success('道具属性已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  async function handleDelete(record: ItemAttributeRecord) {
  ...
  const columns: ColumnsType<ItemAttributeRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: ItemAttributeRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<ItemAttributeRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'item-attribute-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
      ...
      <Modal
        destroyOnHidden
        title={editingRow ? '编辑道具属性' : '新增道具属性'}
        open={modalOpen}
        width={modalWidth}
