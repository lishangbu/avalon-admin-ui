...
import type { MoveAilmentRecord, MoveAilmentQuery, MoveAilmentUpsertInput } from './service'
...
const pageTitle = '招式异常管理'
const pageSubtitle = '对接后端招式异常接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): MoveAilmentQuery {
  const query: MoveAilmentQuery = {}
...
function toFormValues(record?: MoveAilmentRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): MoveAilmentUpsertInput {
  const payload: MoveAilmentUpsertInput = {}
...
export default function DatasetMoveAilmentPage() {
  ...
  const [editingRow, setEditingRow] = useState<MoveAilmentRecord | null>(null)
  const [query, setQuery] = useState<MoveAilmentQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'move-ailment', 'list', query],
  ...
  async function loadData(nextQuery: MoveAilmentQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'move-ailment', 'list', nextQuery],
  ...
  function openEdit(record: MoveAilmentRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('招式异常已更新')
      } else {
        await createRow(payload)
        message.success('招式异常已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  async function handleDelete(record: MoveAilmentRecord) {
  ...
  const columns: ColumnsType<MoveAilmentRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: MoveAilmentRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<MoveAilmentRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'move-ailment-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
      ...
      <Modal
        destroyOnHidden
        title={editingRow ? '编辑招式异常' : '新增招式异常'}
        open={modalOpen}
        width={modalWidth}
