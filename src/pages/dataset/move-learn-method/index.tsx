...
import type { MoveLearnMethodRecord, MoveLearnMethodQuery, MoveLearnMethodUpsertInput } from './service'
...
const pageSubtitle = '对接后端招式学习方式接口，支持列表查询、新增、编辑和删除。'
...
function toSearchQuery(values: SearchValues): MoveLearnMethodQuery {
  const query: MoveLearnMethodQuery = {}
...
function toPayload(values: FormValues): MoveLearnMethodUpsertInput {
  const payload: MoveLearnMethodUpsertInput = {}
...
export default function DatasetMoveLearnMethodPage() {
  ...
  const [editingRow, setEditingRow] = useState<MoveLearnMethodRecord | null>(null)
  const [query, setQuery] = useState<MoveLearnMethodQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'move-learn-method', 'list', query],
  ...
  async function loadData(nextQuery: MoveLearnMethodQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'move-learn-method', 'list', nextQuery],
  ...
  function openEdit(record: MoveLearnMethodRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('招式学习方式已更新')
      } else {
        await createRow(payload)
        message.success('招式学习方式已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  const columns: ColumnsType<MoveLearnMethodRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: MoveLearnMethodRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<MoveLearnMethodRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'move-learn-method-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
