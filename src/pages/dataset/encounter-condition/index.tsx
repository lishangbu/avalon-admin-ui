...
import {
  ...
  listRows,
  updateRow,
  type EncounterConditionQuery,
  type EncounterConditionRecord,
...
function toSearchQuery(values: SearchValues): EncounterConditionQuery {
  const query: EncounterConditionQuery = {}
...
function toPayload(values: FormValues): EncounterConditionUpsertInput {
  return {
    id: values.id,
...
export default function DatasetEncounterConditionPage() {
  ...
  const [editingRow, setEditingRow] = useState<EncounterConditionRecord | null>(null)
  const [query, setQuery] = useState<EncounterConditionQuery>({})
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)

  const rowsQuery = useQuery({
    queryKey: ['dataset', 'encounter-condition', 'list', query],
  ...
  async function loadData(nextQuery: EncounterConditionQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'encounter-condition', 'list', nextQuery],
  ...
  function openEdit(record: EncounterConditionRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('遭遇条件已更新')
      } else {
        await createRow(payload)
        message.success('遭遇条件已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  const columns: ColumnsType<EncounterConditionRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
    ...
    },
    {
      ...
      width: 180,
      fixed: 'right',
      render: (_: unknown, record: EncounterConditionRecord) => (
        <Space size="small">
  ...
  return (
    ...
    >
      ...
      <Table<EncounterConditionRecord>
        rowKey={(record, index) => stringifyId(record.id) ?? stringifyId(record.internalName) ?? 'encounter-condition-' + index}
        loading={loading}
        columns={columns}
        dataSource={rows}
        pagination={{
