...
function toFormValues(user?: UserView | null): UserFormValues {
  return {
    ...
    hashedPassword: '',
    roleIds: collectRelationIds(user?.roles),
  }
}
...
function normalizePayload(
  ...
  const payload: SaveUserInput = {
    ...
    enabled: values.enabled,
    hashedPassword: toOptionalString(values.hashedPassword),
...
function renderRoleTags(roles?: RoleView[] | null) {
  const names = (roles ?? []).map((role) => getRoleDisplayName(role))
...
export default function UserManagementPage() {
  ...
  const canQuery = has(SYSTEM_PERMISSION_CODES.user.query)
  const canUpdate = has(SYSTEM_PERMISSION_CODES.user.update)
  const canDelete = has(SYSTEM_PERMISSION_CODES.user.delete)
  ...
  const rowsQuery = useQuery({
    queryKey: ['system', 'user', 'page', page, pageSize, query],
    queryFn: () =>
      getUserPage({
        page,
        size: pageSize,
  ...
  const roleOptions = buildRoleOptions(roleOptionsQuery.data?.data ?? [])
  ...
  async function loadRows(
    ...
    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['system', 'user', 'page', nextPage, nextPageSize, nextQuery],
        queryFn: () =>
          getUserPage({
            page: nextPage,
            size: nextPageSize,
  ...
  const columns: ColumnsType<UserView> = [
    ...
    },
    {
      ...
      key: 'roles',
      width: 260,
      render: (value?: RoleView[] | null) => renderRoleTags(value),
    },
    {
      title: '头像地址',
      dataIndex: 'avatar',
    ...
    return (
      <PageContainer title="用户管理">
        <Result
          status="403"
          title="无权查看"
          subTitle="当前账号没有 system:user:query 权限，无法查看用户数据。"
        />
      </PageContainer>
    )
  }
