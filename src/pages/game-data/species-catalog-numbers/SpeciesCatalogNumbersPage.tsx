...
export function SpeciesCatalogNumbersPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', speciesCatalogNumbersResource.key, query],
    queryFn: () => speciesCatalogNumbersGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(speciesCatalogNumbersResource, values);
      if (modalMode === 'create') {
        return speciesCatalogNumbersGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return speciesCatalogNumbersGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => speciesCatalogNumbersGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', speciesCatalogNumbersResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(speciesCatalogNumbersResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(speciesCatalogNumbersResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {speciesCatalogNumbersResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {speciesCatalogNumbersResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(speciesCatalogNumbersResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {speciesCatalogNumbersResource.fields
      ...
      <SpeciesCatalogNumbersRecordTable
      ...
      <EntityDrawer
        ...
        title={`${speciesCatalogNumbersResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(speciesCatalogNumbersResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${speciesCatalogNumbersResource.title}`
            : `编辑${speciesCatalogNumbersResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {speciesCatalogNumbersResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function SpeciesCatalogNumbersRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...speciesCatalogNumbersResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(speciesCatalogNumbersResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(speciesCatalogNumbersResource) }}
        pagination={{
