...
export function ItemFlingEffectsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', itemFlingEffectsResource.key, query],
    queryFn: () => itemFlingEffectsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(itemFlingEffectsResource, values);
      if (modalMode === 'create') {
        return itemFlingEffectsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return itemFlingEffectsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => itemFlingEffectsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({ queryKey: ['game-data', itemFlingEffectsResource.key] });
  }

  function openCreateModal() {
    setModalMode('create');
    setEditingRecord(null);
    form.setFieldsValue(createInitialValues(itemFlingEffectsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(itemFlingEffectsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {itemFlingEffectsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">{itemFlingEffectsResource.description}</Typography.Text>
        </div>
        <Space wrap>
          <Button
            type="primary"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(itemFlingEffectsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {itemFlingEffectsResource.fields
      ...
      <EntityDrawer
        ...
        title={`${itemFlingEffectsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(itemFlingEffectsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${itemFlingEffectsResource.title}`
            : `编辑${itemFlingEffectsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {itemFlingEffectsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function ItemFlingEffectsRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...itemFlingEffectsResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(itemFlingEffectsResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(itemFlingEffectsResource) }}
        pagination={{
