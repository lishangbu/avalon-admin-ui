...
export function AdvancedContestEffectSkillsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', advancedContestEffectSkillsResource.key, query],
    queryFn: () => advancedContestEffectSkillsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(advancedContestEffectSkillsResource, values);
      if (modalMode === 'create') {
        return advancedContestEffectSkillsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return advancedContestEffectSkillsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) =>
      advancedContestEffectSkillsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', advancedContestEffectSkillsResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(advancedContestEffectSkillsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(advancedContestEffectSkillsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {advancedContestEffectSkillsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {advancedContestEffectSkillsResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(advancedContestEffectSkillsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {advancedContestEffectSkillsResource.fields
      ...
      <AdvancedContestEffectSkillsRecordTable
      ...
      <EntityDrawer
        ...
        title={`${advancedContestEffectSkillsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(advancedContestEffectSkillsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${advancedContestEffectSkillsResource.title}`
            : `编辑${advancedContestEffectSkillsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {advancedContestEffectSkillsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function AdvancedContestEffectSkillsRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...advancedContestEffectSkillsResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(advancedContestEffectSkillsResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(advancedContestEffectSkillsResource) }}
        pagination={{
