...
export function NatureBattleStylePreferencesPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', natureBattleStylePreferencesResource.key, query],
    queryFn: () => natureBattleStylePreferencesGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(natureBattleStylePreferencesResource, values);
      if (modalMode === 'create') {
        return natureBattleStylePreferencesGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return natureBattleStylePreferencesGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) =>
      natureBattleStylePreferencesGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', natureBattleStylePreferencesResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(natureBattleStylePreferencesResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(natureBattleStylePreferencesResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {natureBattleStylePreferencesResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {natureBattleStylePreferencesResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
          ...
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              ...
              placeholder={searchPlaceholder(
                natureBattleStylePreferencesResource.searchPlaceholder,
              )}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {natureBattleStylePreferencesResource.fields
      ...
      <NatureBattleStylePreferencesRecordTable
      ...
      <EntityDrawer
        ...
        title={`${natureBattleStylePreferencesResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(natureBattleStylePreferencesResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${natureBattleStylePreferencesResource.title}`
            : `编辑${natureBattleStylePreferencesResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {natureBattleStylePreferencesResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function NatureBattleStylePreferencesRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...natureBattleStylePreferencesResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(natureBattleStylePreferencesResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(natureBattleStylePreferencesResource) }}
        pagination={{
