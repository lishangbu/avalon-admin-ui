...
export function SpeciesDetailsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', speciesDetailsResource.key, query],
    queryFn: () => speciesDetailsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(speciesDetailsResource, values);
      if (modalMode === 'create') {
        return speciesDetailsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return speciesDetailsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => speciesDetailsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({ queryKey: ['game-data', speciesDetailsResource.key] });
  }

  function openCreateModal() {
    setModalMode('create');
    setEditingRecord(null);
    form.setFieldsValue(createInitialValues(speciesDetailsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(speciesDetailsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {speciesDetailsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">{speciesDetailsResource.description}</Typography.Text>
        </div>
        <Space wrap>
          <Button
            type="primary"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(speciesDetailsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {speciesDetailsResource.fields
      ...
      <EntityDrawer
        ...
        title={`${speciesDetailsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(speciesDetailsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${speciesDetailsResource.title}`
            : `编辑${speciesDetailsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {speciesDetailsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
