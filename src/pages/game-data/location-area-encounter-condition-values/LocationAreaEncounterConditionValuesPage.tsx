...
export function LocationAreaEncounterConditionValuesPage() {
  ...
  const query = useMemo<GameDataListQuery>(
    () => ({
      ...
      ...normalizeFieldFilters(locationAreaEncounterConditionValuesResource, fieldFilters),
      page: page.current - 1,
      size: page.pageSize,
    }),
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', locationAreaEncounterConditionValuesResource.key, query],
    queryFn: () => locationAreaEncounterConditionValuesGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(locationAreaEncounterConditionValuesResource, values);
      if (modalMode === 'create') {
        return locationAreaEncounterConditionValuesGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return locationAreaEncounterConditionValuesGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) =>
      locationAreaEncounterConditionValuesGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', locationAreaEncounterConditionValuesResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(locationAreaEncounterConditionValuesResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(locationAreaEncounterConditionValuesResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {locationAreaEncounterConditionValuesResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {locationAreaEncounterConditionValuesResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
          ...
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              ...
              placeholder={searchPlaceholder(
                locationAreaEncounterConditionValuesResource.searchPlaceholder,
              )}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {locationAreaEncounterConditionValuesResource.fields
      ...
      <LocationAreaEncounterConditionValuesRecordTable
      ...
      <EntityDrawer
        ...
        title={`${locationAreaEncounterConditionValuesResource.title}详情`}
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${locationAreaEncounterConditionValuesResource.title}`
            : `编辑${locationAreaEncounterConditionValuesResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {locationAreaEncounterConditionValuesResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function LocationAreaEncounterConditionValuesRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...locationAreaEncounterConditionValuesResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(locationAreaEncounterConditionValuesResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(locationAreaEncounterConditionValuesResource) }}
        pagination={{
