...
export function BerryFlavorPotenciesPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', berryFlavorPotenciesResource.key, query],
    queryFn: () => berryFlavorPotenciesGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(berryFlavorPotenciesResource, values);
      if (modalMode === 'create') {
        return berryFlavorPotenciesGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return berryFlavorPotenciesGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => berryFlavorPotenciesGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', berryFlavorPotenciesResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(berryFlavorPotenciesResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(berryFlavorPotenciesResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {berryFlavorPotenciesResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {berryFlavorPotenciesResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(berryFlavorPotenciesResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {berryFlavorPotenciesResource.fields
      ...
      <BerryFlavorPotenciesRecordTable
      ...
      <EntityDrawer
        ...
        title={`${berryFlavorPotenciesResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(berryFlavorPotenciesResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${berryFlavorPotenciesResource.title}`
            : `编辑${berryFlavorPotenciesResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {berryFlavorPotenciesResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function BerryFlavorPotenciesRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...berryFlavorPotenciesResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(berryFlavorPotenciesResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(berryFlavorPotenciesResource) }}
        pagination={{
