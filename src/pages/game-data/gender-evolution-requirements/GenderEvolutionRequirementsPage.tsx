...
export function GenderEvolutionRequirementsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', genderEvolutionRequirementsResource.key, query],
    queryFn: () => genderEvolutionRequirementsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(genderEvolutionRequirementsResource, values);
      if (modalMode === 'create') {
        return genderEvolutionRequirementsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return genderEvolutionRequirementsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) =>
      genderEvolutionRequirementsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', genderEvolutionRequirementsResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(genderEvolutionRequirementsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(genderEvolutionRequirementsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {genderEvolutionRequirementsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {genderEvolutionRequirementsResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(genderEvolutionRequirementsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {genderEvolutionRequirementsResource.fields
      ...
      <GenderEvolutionRequirementsRecordTable
      ...
      <EntityDrawer
        ...
        title={`${genderEvolutionRequirementsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(genderEvolutionRequirementsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${genderEvolutionRequirementsResource.title}`
            : `编辑${genderEvolutionRequirementsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {genderEvolutionRequirementsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function GenderEvolutionRequirementsRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...genderEvolutionRequirementsResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(genderEvolutionRequirementsResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(genderEvolutionRequirementsResource) }}
        pagination={{
