...
export function LocationAreaMethodRatesPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', locationAreaMethodRatesResource.key, query],
    queryFn: () => locationAreaMethodRatesGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(locationAreaMethodRatesResource, values);
      if (modalMode === 'create') {
        return locationAreaMethodRatesGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return locationAreaMethodRatesGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) =>
      locationAreaMethodRatesGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', locationAreaMethodRatesResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(locationAreaMethodRatesResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(locationAreaMethodRatesResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {locationAreaMethodRatesResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {locationAreaMethodRatesResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(locationAreaMethodRatesResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {locationAreaMethodRatesResource.fields
      ...
      <LocationAreaMethodRatesRecordTable
      ...
      <EntityDrawer
        ...
        title={`${locationAreaMethodRatesResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(locationAreaMethodRatesResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${locationAreaMethodRatesResource.title}`
            : `编辑${locationAreaMethodRatesResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {locationAreaMethodRatesResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function LocationAreaMethodRatesRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...locationAreaMethodRatesResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(locationAreaMethodRatesResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(locationAreaMethodRatesResource) }}
        pagination={{
