...
export function SkillLearnMethodsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', skillLearnMethodsResource.key, query],
    queryFn: () => skillLearnMethodsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(skillLearnMethodsResource, values);
      if (modalMode === 'create') {
        return skillLearnMethodsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return skillLearnMethodsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => skillLearnMethodsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', skillLearnMethodsResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(skillLearnMethodsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(skillLearnMethodsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {skillLearnMethodsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {skillLearnMethodsResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(skillLearnMethodsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {skillLearnMethodsResource.fields
      ...
      <EntityDrawer
        ...
        title={`${skillLearnMethodsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(skillLearnMethodsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${skillLearnMethodsResource.title}`
            : `编辑${skillLearnMethodsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {skillLearnMethodsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
