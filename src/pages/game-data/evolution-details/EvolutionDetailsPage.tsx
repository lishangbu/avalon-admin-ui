...
export function EvolutionDetailsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', evolutionDetailsResource.key, query],
    queryFn: () => evolutionDetailsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(evolutionDetailsResource, values);
      if (modalMode === 'create') {
        return evolutionDetailsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return evolutionDetailsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => evolutionDetailsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({ queryKey: ['game-data', evolutionDetailsResource.key] });
  }

  function openCreateModal() {
    setModalMode('create');
    setEditingRecord(null);
    form.setFieldsValue(createInitialValues(evolutionDetailsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(evolutionDetailsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {evolutionDetailsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">{evolutionDetailsResource.description}</Typography.Text>
        </div>
        <Space wrap>
          <Button
            type="primary"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(evolutionDetailsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {evolutionDetailsResource.fields
      ...
      <EntityDrawer
        ...
        title={`${evolutionDetailsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(evolutionDetailsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${evolutionDetailsResource.title}`
            : `编辑${evolutionDetailsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {evolutionDetailsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
