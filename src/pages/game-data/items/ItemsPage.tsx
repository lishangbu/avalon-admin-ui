...
export function ItemsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', itemsResource.key, query],
    queryFn: () => itemsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(itemsResource, values);
      if (modalMode === 'create') {
        return itemsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return itemsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => itemsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({ queryKey: ['game-data', itemsResource.key] });
  }

  function openCreateModal() {
    setModalMode('create');
    setEditingRecord(null);
    form.setFieldsValue(createInitialValues(itemsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(itemsResource, record));
    setModalOpen(true);
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <Typography.Text type="secondary">{itemsResource.description}</Typography.Text>
        </div>
        <Space wrap>
          <Button
            type="primary"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(itemsResource.searchPlaceholder)}
      ...
      <EntityDrawer
        ...
        title={`${itemsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(itemsResource, detailRecord, referenceLookup)}
      />

      <Modal
        open={modalOpen}
        title={modalMode === 'create' ? `新建${itemsResource.title}` : `编辑${itemsResource.title}`}
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {itemsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
