...
export function NaturesPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', naturesResource.key, query],
    queryFn: () => naturesGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(naturesResource, values);
      if (modalMode === 'create') {
        return naturesGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return naturesGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => naturesGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({ queryKey: ['game-data', naturesResource.key] });
  }

  function openCreateModal() {
    setModalMode('create');
    setEditingRecord(null);
    form.setFieldsValue(createInitialValues(naturesResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(naturesResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {naturesResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">{naturesResource.description}</Typography.Text>
        </div>
        <Space wrap>
          <Button
            type="primary"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(naturesResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {naturesResource.fields
      ...
      <EntityDrawer
        ...
        title={`${naturesResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(naturesResource, detailRecord, referenceLookup)}
      />

      <Modal
        open={modalOpen}
        title={
          modalMode === 'create' ? `新建${naturesResource.title}` : `编辑${naturesResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {naturesResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
...
function NaturesRecordTable({
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...naturesResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(naturesResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(naturesResource) }}
        pagination={{
