...
export function StatCharacteristicsPage() {
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', statCharacteristicsResource.key, query],
    queryFn: () => statCharacteristicsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(statCharacteristicsResource, values);
      if (modalMode === 'create') {
        return statCharacteristicsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return statCharacteristicsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => statCharacteristicsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', statCharacteristicsResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(statCharacteristicsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(statCharacteristicsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {statCharacteristicsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {statCharacteristicsResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(statCharacteristicsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {statCharacteristicsResource.fields
      ...
      <StatCharacteristicsRecordTable
      ...
      <EntityDrawer
        ...
        title={`${statCharacteristicsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(statCharacteristicsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${statCharacteristicsResource.title}`
            : `编辑${statCharacteristicsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {statCharacteristicsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
 ...
 */
function StatCharacteristicsRecordTable({
  rows,
  totalRowCount,
  page,
  loading,
  ...
  const columns: ColumnsType<GameDataRecord> = [
    ...statCharacteristicsResource.fields.map((field, index) => ({
    ...
    })),
    {
      ...
      render: (_: unknown, record: GameDataRecord) => (
        <Space size="small">
          ...
          <Popconfirm
            title="删除资料"
            description={`确认删除${formatRecordTitle(statCharacteristicsResource, record, referenceLookup)}？`}
            okText="确认"
            cancelText="取消"
  ...
  return (
    <Card size="small">
      ...
      <Table<GameDataRecord>
        ...
        dataSource={rows}
        loading={loading}
        scroll={{ x: tableScrollWidth(statCharacteristicsResource) }}
        pagination={{
