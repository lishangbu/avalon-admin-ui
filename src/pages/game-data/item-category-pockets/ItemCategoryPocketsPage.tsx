...
export function ItemCategoryPocketsPage() {
  ...
  const query = useMemo<GameDataListQuery>(
    () => ({
      ...
      ...normalizeFieldFilters(itemCategoryPocketsResource, fieldFilters),
      page: page.current - 1,
      size: page.pageSize,
    }),
  ...
  const recordsQuery = useQuery({
    queryKey: ['game-data', itemCategoryPocketsResource.key, query],
    queryFn: () => itemCategoryPocketsGameDataService.list(query),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (values: GameDataFormValues) => {
      const fields = toRecordFields(itemCategoryPocketsResource, values);
      if (modalMode === 'create') {
        return itemCategoryPocketsGameDataService.create(fields);
      }
      if (!editingRecord) {
        throw new Error('缺少正在编辑的资料');
      }
      return itemCategoryPocketsGameDataService.update(editingRecord.id, fields);
    },
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (record: GameDataRecord) => itemCategoryPocketsGameDataService.remove(record.id),
    onSuccess: async () => {
  ...
  function invalidateRecords() {
    return queryClient.invalidateQueries({
      queryKey: ['game-data', itemCategoryPocketsResource.key],
    });
  }
  ...
  function openCreateModal() {
    ...
    form.setFieldsValue(createInitialValues(itemCategoryPocketsResource));
    setModalOpen(true);
  }
  ...
  function openEditModal(record: GameDataRecord) {
    ...
    form.setFieldsValue(toFormValues(itemCategoryPocketsResource, record));
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Typography.Title level={3} className="!mb-1">
            {itemCategoryPocketsResource.title}
          </Typography.Title>
          <Typography.Text type="secondary">
            {itemCategoryPocketsResource.description}
          </Typography.Text>
        </div>
        <Space wrap>
          <Button
        ...
        <div className="flex flex-wrap items-end gap-3">
          <Form.Item label="关键字" className="!mb-0">
            <Input.Search
              allowClear
              value={keyword}
              placeholder={searchPlaceholder(itemCategoryPocketsResource.searchPlaceholder)}
              onChange={(event) => updateKeyword(event.target.value)}
              onSearch={(value) => searchKeyword(value)}
            />
          </Form.Item>
          {itemCategoryPocketsResource.fields
      ...
      <ItemCategoryPocketsRecordTable
      ...
      <EntityDrawer
        ...
        title={`${itemCategoryPocketsResource.title}详情`}
        onClose={() => setDetailRecord(null)}
        items={detailItems(itemCategoryPocketsResource, detailRecord, referenceLookup)}
      />
      ...
      <Modal
        ...
        title={
          modalMode === 'create'
            ? `新建${itemCategoryPocketsResource.title}`
            : `编辑${itemCategoryPocketsResource.title}`
        }
        okText="保存"
        cancelText="取消"
      ...
      >
        ...
        >
          {itemCategoryPocketsResource.fields.map((field) => (
            <Form.Item
              key={field.name}
              name={field.name}
