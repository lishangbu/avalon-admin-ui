...
export default function DatasetItemCategoryPage() {
  ...
  const itemPocketQuery = useQuery({
    queryKey: ['dataset', 'item-pocket', 'list'],
    queryFn: () => listItemPocketRows(),
  })
  const rowsQuery = useQuery({
    queryKey: ['dataset', 'item-category', 'list', query],
    queryFn: () => listRows(query),
  })

  const loading = rowsQuery.isFetching
  const optionLoading = itemPocketQuery.isFetching
  const rows = rowsQuery.data ?? []
  const total = rows.length
  const itemPocketOptions = toSelectOptions(itemPocketQuery.data ?? [])
  ...
  async function loadData(nextQuery: ItemCategoryQuery = query) {
    ...
    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'item-category', 'list', nextQuery],
    ...
    <PageContainer
      ...
      extra={[
        ...
        <Button
          ...
          onClick={() =>
            void Promise.all([itemPocketQuery.refetch(), rowsQuery.refetch()])
          }
        >
          刷新
        </Button>,
