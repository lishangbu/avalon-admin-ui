...
export default function DatasetBerryPage() {
  ...
  const berryFirmnessQuery = useQuery({
    queryKey: ['dataset', 'berry-firmness', 'list'],
    queryFn: () => listBerryFirmnessRows(),
  })
  ...
  const rowsQuery = useQuery({
    queryKey: ['dataset', 'berry', 'page', page, pageSize, query],
    queryFn: () =>
      getPage({
        page,
        size: pageSize,
  ...
  const optionLoading = berryFirmnessQuery.isFetching || typeQuery.isFetching
  const rows = rowsQuery.data?.items ?? []
  const total = rowsQuery.data?.totalItems ?? 0
  const berryFirmnessOptions = toSelectOptions(berryFirmnessQuery.data ?? [])
