...
export default function DatasetItemPage() {
  ...
  const itemFlingEffectQuery = useQuery({
    queryKey: ['dataset', 'item-fling-effect', 'list'],
    queryFn: () => listItemFlingEffectRows(),
  })
  const itemAttributeQuery = useQuery({
    queryKey: ['dataset', 'item-attribute', 'list'],
    queryFn: () => listItemAttributeRows(),
  })
  const rowsQuery = useQuery({
    queryKey: ['dataset', 'item', 'page', page, pageSize, query],
    queryFn: () =>
      getPage({
        page,
        size: pageSize,
  ...
  const optionLoading =
    itemFlingEffectQuery.isFetching || itemAttributeQuery.isFetching
  const rows = rowsQuery.data?.items ?? []
  const total = rowsQuery.data?.totalItems ?? 0
  const itemFlingEffectOptions = toSelectOptions(
    itemFlingEffectQuery.data ?? [],
  )
  const itemAttributeOptions = toSelectOptions(itemAttributeQuery.data ?? [])
    ...
    <PageContainer
      ...
      extra={[
        ...
        <Button
          ...
          onClick={() =>
            void Promise.all([
              itemFlingEffectQuery.refetch(),
              itemAttributeQuery.refetch(),
              rowsQuery.refetch(),
            ])
          }
        >
