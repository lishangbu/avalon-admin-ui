...
function toSearchQuery(values: SearchValues): EvolutionTriggerQuery {
  const query: EvolutionTriggerQuery = {}
...
function toFormValues(record?: EvolutionTriggerRecord | null): FormValues {
  return {
...
function toPayload(values: FormValues): EvolutionTriggerUpsertInput {
  const payload: EvolutionTriggerUpsertInput = {}
...
export default function DatasetEvolutionTriggerPage() {
  ...
  const rowsQuery = useQuery({
    queryKey: ['dataset', 'evolution-trigger', 'list', query],
  ...
  async function loadData(nextQuery: EvolutionTriggerQuery = query) {
    const isSameQuery = JSON.stringify(nextQuery) === JSON.stringify(query)

    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['dataset', 'evolution-trigger', 'list', nextQuery],
  ...
  function openEdit(record: EvolutionTriggerRecord) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.id) {
        ...
        message.success('进化触发方式已更新')
      } else {
        await createRow(payload)
        message.success('进化触发方式已创建')
      }
      setModalOpen(false)
      setEditingRow(null)
      form.resetFields()
  ...
  async function handleDelete(record: EvolutionTriggerRecord) {
  ...
  const columns: ColumnsType<EvolutionTriggerRecord> = [
    {
      title: '名称',
      dataIndex: 'name',
      key: 'name',
  ...
  return (
    <PageContainer
      title="进化触发方式管理"
      subTitle="对接后端进化触发方式接口，支持列表查询、新增、编辑和删除。"
      extra={[
        <Button
          key="create"
          type="primary"
    ...
    >
      ...
      <Modal
        destroyOnHidden
        title={editingRow ? '编辑进化触发方式' : '新增进化触发方式'}
        open={modalOpen}
