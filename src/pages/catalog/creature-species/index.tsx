...
export default function DatasetCreatureSpeciesPage() {
  ...
  const growthRateQuery = useQuery({
    queryKey: ['dataset', 'growth-rate', 'list'],
    queryFn: () => listGrowthRateRows(),
  })
  const creatureColorQuery = useQuery({
    queryKey: ['dataset', 'creature-color', 'list'],
    queryFn: () => listCreatureColorRows(),
  })
  const creatureHabitatQuery = useQuery({
    queryKey: ['dataset', 'creature-habitat', 'list'],
    queryFn: () => listCreatureHabitatRows(),
  })
  const creatureShapeQuery = useQuery({
    queryKey: ['dataset', 'creature-shape', 'list'],
    queryFn: () => listCreatureShapeRows(),
  })
  const creatureSpeciesOptionsQuery = useQuery({
    queryKey: ['dataset', 'creature-species', 'page', 'options'],
    queryFn: () =>
      getPage({
        page: 1,
        size: 2000,
  ...
  const rowsQuery = useQuery({
    queryKey: ['dataset', 'creature-species', 'page', page, pageSize, query],
    queryFn: () =>
      getPage({
        page,
        size: pageSize,
  ...
  const growthRateOptions = toSelectOptions(growthRateQuery.data ?? [])
  const creatureColorOptions = toSelectOptions(creatureColorQuery.data ?? [])
  const creatureHabitatOptions = toSelectOptions(
    creatureHabitatQuery.data ?? [],
  )
  const creatureShapeOptions = toSelectOptions(creatureShapeQuery.data ?? [])
  const creatureSpeciesOptions = toSelectOptions(
    creatureSpeciesOptionsQuery.data?.items ?? [],
  )
