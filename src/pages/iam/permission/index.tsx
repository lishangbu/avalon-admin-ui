...
function toFormValues(
  ...
  return {
    ...
    menuId: permission?.menuId ?? permission?.menu?.id ?? '',
...
export default function PermissionManagementPage() {
  ...
  const canQuery = has(IAM_PERMISSION_CODES.permission.query)
  const canUpdate = has(IAM_PERMISSION_CODES.permission.update)
  const canDelete = has(IAM_PERMISSION_CODES.permission.delete)
  ...
  const rowsQuery = useQuery({
    queryKey: ['iam', 'permission', 'page', page, pageSize, query],
    queryFn: () =>
      getPermissionPage({
        page,
        size: pageSize,
  ...
  const rawRows = useMemo(() => rowsQuery.data?.items ?? [], [rowsQuery.data])
  ...
  const rows = useMemo(
    () =>
      rawRows.map((permission) => ({
        ...permission,
        menu:
          permission.menu ??
          (permission.menuId ? menuById.get(permission.menuId) : undefined),
      })),
    [menuById, rawRows],
  )
  ...
  async function loadRows(
    ...
    nextQuery: PermissionQuery = query,
  ) {
    const isSameQuery =
      nextPage === page &&
      nextPageSize === pageSize &&
    ...
    return (
      <PageContainer title="权限管理">
        <Result
          status="403"
          title="无权查看"
          subTitle="当前账号没有 iam:permission:query 权限，无法查看权限点数据。"
        />
      </PageContainer>
    )
  }
  ...
  return (
    <PageContainer
      ...
      extra={[
        <PermissionGuard
          key="add"
          permission={IAM_PERMISSION_CODES.permission.create}
        >
          <Button
            type="primary"
            icon={<PlusOutlined />}
        ...
        <Button
          ...
          icon={<ReloadOutlined />}
          onClick={() =>
            void Promise.all([menuTreeQuery.refetch(), rowsQuery.refetch()])
          }
        >
          刷新
        </Button>,
    ...
    >
      ...
      <Table<PermissionView>
        rowKey={(record) => record.id || record.code || 'iam-permission-row'}
      ...
      >
        <Form form={form} layout="vertical" initialValues={toFormValues()}>
          ...
          >
            <Input placeholder="例如：iam:user:create" />
          </Form.Item>
          <Form.Item
            name="name"
            label="权限名称"
