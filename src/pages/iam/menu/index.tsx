...
export default function MenuManagementPage() {
  ...
  const canQuery = has(IAM_PERMISSION_CODES.menu.query)
  const canCreate = has(IAM_PERMISSION_CODES.menu.create)
  const canUpdate = has(IAM_PERMISSION_CODES.menu.update)
  const canDelete = has(IAM_PERMISSION_CODES.menu.delete)
  const treeQuery = useQuery({
    queryKey: ['iam', 'menu', 'tree'],
    queryFn: () => listMenuTree(),
    enabled: canQuery,
  })
  const loading = canQuery ? treeQuery.isFetching || detailLoading : false
  ...
  const openEdit = useCallback(
    async (record: MenuTreeNode) => {
      ...
      try {
        const result = await getMenuById(String(record.id))
        form.resetFields()
    ...
    return (
      <PageContainer title="菜单管理">
        <Result
          status="403"
          title="无权查看"
          subTitle="当前账号没有 iam:menu:query 权限，无法查看菜单数据。"
        />
      </PageContainer>
    )
  }
  ...
  return (
    <PageContainer
      ...
      extra={[
        <PermissionGuard
          key="add-root"
          permission={IAM_PERMISSION_CODES.menu.create}
        >
          <Button
            type="primary"
            icon={<PlusOutlined />}
      ...
      >
        <Form form={form} layout="vertical" initialValues={toFormValues()}>
          ...
          <Form.Item name="path" label="路径">
            <Input placeholder="/iam/menu" />
          </Form.Item>
          <Form.Item name="component" label="组件标识">
            <Input placeholder="iam/menu/index" />
          </Form.Item>
