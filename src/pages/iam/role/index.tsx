...
import { listMenuTree } from '@/pages/iam/menu/service'
import { lookupPermissions } from '@/pages/iam/permission/service'
...
} from '@/pages/iam/shared/tree-options'
import { flattenMenuTree } from '@/pages/iam/shared/idm-normalizers'
...
function toFormValues(role?: RoleView | null): RoleFormValues {
  return {
    ...
    menuIds: role?.menuIds ?? collectRelationIds(role?.menus),
    permissionIds: role?.permissionIds ?? collectRelationIds(role?.permissions),
  }
}
...
function renderMenuTags(
  ...
  menuById: Map<string, MenuView>,
) {
  const names = (menuIds ?? []).map((id) =>
    getMenuDisplayName(menuById.get(id) ?? { id }),
  )
...
function renderPermissionTags(
  ...
  permissionById: Map<string, PermissionView>,
) {
  const names = (permissionIds ?? []).map((id) =>
    getPermissionDisplayName(permissionById.get(id) ?? { id }),
  )
...
export default function RoleManagementPage() {
  ...
  const canQuery = has(IAM_PERMISSION_CODES.role.query)
  const canUpdate = has(IAM_PERMISSION_CODES.role.update)
  const canDelete = has(IAM_PERMISSION_CODES.role.delete)
  ...
  const permissionListQuery = useQuery({
    queryKey: ['iam', 'permission', 'lookup', {}],
    queryFn: () => lookupPermissions({}),
    enabled: canQuery,
  })
  const rowsQuery = useQuery({
    queryKey: ['iam', 'role', 'page', page, pageSize, query],
    queryFn: () =>
      getRolePage({
        page,
        size: pageSize,
  ...
  const menuById = useMemo(() => {
    return new Map(
      flattenMenuTree(menuTreeQuery.data ?? []).flatMap((menu) =>
        menu.id ? [[menu.id, menu] as const] : [],
      ),
    )
  }, [menuTreeQuery.data])
  const permissionById = useMemo(() => {
    return new Map(
      permissionRows.flatMap((permission) =>
        permission.id ? [[permission.id, permission] as const] : [],
      ),
    )
  }, [permissionRows])
  ...
  async function loadRows(
    ...
    if (!isSameQuery) {
      await queryClient.ensureQueryData({
        queryKey: ['iam', 'role', 'page', nextPage, nextPageSize, nextQuery],
        queryFn: () =>
          getRolePage({
            page: nextPage,
            size: nextPageSize,
  ...
  const columns: ColumnsType<RoleView> = [
    ...
    },
    {
      title: '菜单权限',
      key: 'menus',
      render: (_, record) => renderMenuTags(record.menuIds, menuById),
    },
    {
      title: '按钮权限',
      key: 'permissions',
      render: (_, record) =>
        renderPermissionTags(record.permissionIds, permissionById),
    },
    {
      title: '操作',
      key: 'actions',
    ...
    return (
      <PageContainer title="角色管理">
        <Result
          status="403"
          title="无权查看"
          subTitle="当前账号没有 iam:role:query 权限，无法查看角色数据。"
        />
      </PageContainer>
    )
  }
  ...
  return (
    <PageContainer
      ...
      extra={[
        <PermissionGuard
          key="add"
          permission={IAM_PERMISSION_CODES.role.create}
        >
          <Button
            type="primary"
            icon={<PlusOutlined />}
    ...
    >
      ...
      <Table<RoleView>
        rowKey={(record) => record.id || record.code || 'iam-role-row'}
