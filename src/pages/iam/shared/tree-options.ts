...
export function buildPermissionTreeData(
  ...
  menuById?: Map<string, Partial<MenuView>>,
): TreeSelectOption[] {
  ...
  permissions.forEach((permission) => {
    ...
    const menu =
      permission.menu ??
      (hasId(permission.menuId) ? menuById?.get(permission.menuId) : undefined)
    const menuId = hasId(menu?.id)
      ? menu.id
      : hasId(permission.menuId)
        ? permission.menuId
        : `menu-${permissionId}`
